...
from nets.keras.common.blocks import SEBlock


def XBlock(in_tensor, filter_list, kernel_size, strides, groups, activation, weight_decay):
    '''
    ...
    conv2d_2_out_tensor = Conv2dBnAct(conv2d_1_out_tensor, filter_list[1], kernel_size, strides, groups=groups,
...
def YBlock(in_tensor, filter_list, kernel_size, strides, groups, activation, weight_decay, se_reduction_ratio):
    '''
    :param in_tensor: Input tensor
    :param filter_list: Conv2d filter number list
    :param kernel_size: Conv2d filter size
    ...
    conv2d_2_out_tensor = Conv2dBnAct(conv2d_1_out_tensor, filter_list[1], kernel_size, strides, groups=groups,
                                      activation=activation,
                                      weight_decay=weight_decay)
    se_out_tensor = SEBlock(conv2d_2_out_tensor, activation, weight_decay, se_reduction_ratio)
    conv2d_3_out_tensor = Conv2dBnAct(se_out_tensor, filter_list[2], (1, 1), (1, 1), activation=activation,
                                      weight_decay=weight_decay)
    if conv2d_3_out_tensor.shape[1:] == in_tensor.shape[1:]:
        identity = in_tensor
    else:
...
if __name__ == "__main__":
    ...
    model = XBlock(input_tensor, [32, 16, 32], (3, 3), (1, 1), groups=4, activation="relu", weight_decay=weight_decay)
